...
function init($argv){
    ...
    if(!is_dir("../app/config")){
        ...
        $arquivo = fopen("../app/config/info.txt", 'w');
        if($arquivo == false){
            echo "Error: Falha ao criar arquivo de configuração (info). \n";
            return null;
        }else{
            $texto = "Essa pasta contém o arquivo de configuração do projeto, atente-se no preenchimento \ndos campos necessários para correto funcionamento do software.";
            $escrever = fwrite($arquivo, $texto);
            if($escrever == false){
                echo "Error: Falha ao preencher arquivo de configuração (info). \n";
                return null;
            }else{
                echo "Arquivo 'info' criado com sucesso. \n";
            }
        } 
        ...
        if($arquivo == false){
            ...
            require "./cmd/snippets/config.php";
    ...
    if(!is_dir("../app/database")){
        ...
        if($arquivo == false){
            ...
            require "./cmd/snippets/database.php";
    ...
    if(!is_dir("../")){
        ...
        if($arquivo == false){
            ...
            require "./cmd/snippets/htaccess.php";
    ...
    if(!is_dir("../")){
        ...
        if($arquivo == false){
            ...
            require "./cmd/snippets/index.php";
            $texto = indexSnippet();
            $escrever = fwrite($arquivo, $texto);
            if($escrever == false){
    ...
    if(!is_dir("../app/routes")){
        ...
        if($arquivo == false){
            ...
            require "./cmd/snippets/routes.php";
            $texto = routesSnippet();
            $escrever = fwrite($arquivo, $texto);
            if($escrever == false){
