        ...
        if($auth->status == 0){
            ...
            $bodyCheckFields = ImpetusUtils::bodyCheckFields(
                [
                    ...
                    ["enderecoBairro", $jsonParams->enderecoCep, ['type(string)', 'uppercase', 'length(1024)', 'nullable']],
                ]
            );
            ...
            $data = [
                ...
                "enderecoBairro" => $jsonParams->enderecoBairro,
                "updatedAt" => $datetime
            ];

            //Atualiza dados
            $request = Pessoas::updatePessoas($data);
            if($request->status == 0){
                $response = [
                    "code" => "400 Bad request",
                    "response" => $request
