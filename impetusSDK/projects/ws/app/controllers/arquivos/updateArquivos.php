        ...
        if($auth->status == 0){
            ...
            $bodyCheckFields = ImpetusUtils::bodyCheckFields(
                [
					...
					["nome", $jsonParams->nome, ['type(string)', 'uppercase', 'length(1000)', 'nullable']],
					["path", $jsonParams->path, ['type(string)', 'uppercase', 'length(5000)', 'nullable']],
            ...
            $data = [
                ...
                "status" => $jsonParams->status,
						...
						"path" => $jsonParams->path,
						"usuarioId" => $jsonParams->usuarioId,
                "updatedAt" => $datetime
            ];

            //Atualiza dados
            $request = Arquivos::updateArquivos($data);
            if($request->status == 0){
                $response = [
                    "code" => "400 Bad request",
                    "response" => $request
