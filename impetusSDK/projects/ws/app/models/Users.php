...
class Users
{
    ...
    static function listUsers($data)
    {
        ...
        if(isset($data["pessoaId"]) && !empty($data["pessoaId"])) {
            $query .= $clausule . "pessoaId = ". $data["pessoaId"];
            $clausule = " AND ";
        }
    ...
    {
        require "../config.php";
        $stmt = $conn->prepare("INSERT INTO users (status, name, email, username, password, permission, isConfirmedEmail, pessoaId) VALUES(:STATUS, :NAME, :EMAIL, :USERNAME, :PASSWORD, :PERMISSION, :ISCONFIRMEDEMAIL, :PESSOAID)");
        ...
        $stmt->bindParam(":PESSOAID", $data["pessoaId"], \PDO::PARAM_INT);
    ...
    {
        require "../config.php";
        $stmt = $conn->prepare("UPDATE users SET status = :STATUS, name = :NAME, email = :EMAIL, username = :USERNAME, password = :PASSWORD, permission = :PERMISSION, isConfirmedEmail = :ISCONFIRMEDEMAIL, updatedAt = :UPDATEDAT, pessoaId = :PESSOAID WHERE id = :ID");
