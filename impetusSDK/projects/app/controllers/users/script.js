...
listDatatable = (currentPage = 1) => {
    ...
    //Buscar dados
    axios.get($("#webservicePath").val() + `users/list`, {
        params: {
...
storeItems = () => {
    axios.post($("#webservicePath").val() + `users/create`, {
        ...
        pessoaId: $("#form-create-users-pessoaid").val(),
        status: 'ACTIVE'
    }, {
        headers: { 
            Authorization : `Bearer ` + $("#sessionToken").val()
...
readItems = (id) => {
    axios.get($("#webservicePath").val() + `users/get`, {
        params: {
            id: id
        },
        headers: { 
        ...
        if (response.data.status == 0) {
            ...
            $("#form-view-users-pessoaNome").val(response.data.data.pessoaNome)
            $("#form-view-users-pessoaId").val(response.data.data.pessoaId)
    ...
    setTimeout(() => {
        axios.get($("#webservicePath").val() + `users/get`, {
            ...
            if (response.data.status == 0) {
                ...
                $("#select-item-update").val(response.data.data.pessoaNome)
                $("#form-edit-users-pessoaid").val(response.data.data.pessoaId)
                $("#form-edit-users-pessoaid-filter").val(response.data.data.pessoaNome)
...
updateItems = () => {
    axios.put($("#webservicePath").val() + `users/update`, {
        ...
        pessoaId: $("#form-edit-users-pessoaid").val(),
...
destroyItems = () => {
    axios.delete($("#webservicePath").val() + `users/delete`, { 
        params: {
